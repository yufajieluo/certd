...
async function spawn(opts: SpawnOption): Promise<string> {
  ...
  cmd = cmd.replace(/--eab\s*"[^"]*"/, '--eab');
  log.info(`执行命令: ${cmd}`);

  // 写入到临时脚本文件
  const scriptPath = join(tmpdir(), `temp_script_${Date.now()}.sh`);
  writeFileSync(scriptPath, cmd);
  log.info(`---------- scriptPath is ${scriptPath}`)

  let stdout = "";
  let stderr = "";
  return safePromise((resolve, reject) => {
    const ls = childProcess.spawn("sh", [scriptPath], {
      shell: true,
      env: {
        ...process.env,
        ...opts.env,
